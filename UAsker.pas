...
procedure TFAsker.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFAsker.VazgecClick(Sender: TObject);
begin
Close;
end;


procedure TFAsker.YaziBtClick(Sender: TObject);
var ayy:string;
begin
 Case  StrToInt(Edit2.Text)     of
    1: ayy:=FData.UniAcilisCalYilname.value+' OCAK';
    2: ayy:=FData.UniAcilisCalYilname.value+' ÞUBAT';
...
procedure TFAsker.HazirBtClick(Sender: TObject);
begin
i:=0;
Query2.Close;
...
while not Query2.eof do
 ...
 Begin
  ...
  Query2.FieldByName('Adi').asstring+' ' +Query2.FieldByName('Soyadi').asstring;
  Table1.FieldByName('Banhes').value:= Query2.FieldByName('SskNo').value;
  Table1.FieldByName('Net').value:=Query2.FieldByName('Pkaz34').value;
  Table1.Post;
  end else
  begin
  Table1.Edit;
  Table1.FieldByName('Net').value:=Table1.FieldByName('Net').AsCurrency+
  Query2.FieldByName('Pkaz34').value;
  Table1.Post;
 end;
 Query2.Next;
 END;
...
procedure TFAsker.QRBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
...
procedure TFAsker.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
