...
//    UniConGuncelleme.Server := '213.14.165.53,3575';
    ...
    UniConGuncelleme.Username := 'GenelTahakkukUDATE';
    UniConGuncelleme.Password :='{8F3250E4-F380-41AE-946B-2F24E474CD0F}';
    UniConGuncelleme.Database := 'GenelTahakkukUPDATE';
    TDONEM.Open;
  end;
  TKATSAY.Open;
  TKatoran.Open;
...
begin
  if messagebox(handle, 'Veritabaný iþlemleri güncellenecek', 'GÜNCELLEME',
   mb_YesNo + MB_ICONQUESTION) = mrYes then
  begin
    ScrGuncelleme.Execute;
    ShowMessage('Güncelemeler tamamlandý. Lütfen programý yeniden baþlatýn');
  end;
end;
...
begin
  ...
  begin
    if messagebox(handle, 'Deðiþikler Kaydedilsin mi?', 'KAPANIÞ',
      mb_YesNo + MB_ICONQUESTION) <> mrYes then
    begin
